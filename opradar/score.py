...
from . import bench_gen, eligibility, features, match, people
...
def report(ranked: pd.DataFrame, plan: pd.DataFrame, value: pd.DataFrame,
    ...
    for r in ranked.head(15).itertuples():
        ...
        aged = r.now_aged_open if r.now_aged_open == r.now_aged_open else "-"
        stock = r.now_it_stock if r.now_it_stock == r.now_it_stock else "-"
        out.append(
